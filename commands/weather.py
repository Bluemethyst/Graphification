...
class Weather(commands.Cog):
    ...
    async def hourly_forecast(
        ...
        # fetch api stuff
        raw = httpx.get(
            f"https://api.open-meteo.com/v1/forecast?latitude={lat}&longitude={long}&hourly=temperature_2m,relative_humidity_2m,dew_point_2m,rain&timezone=Pacific%2FAuckland&forecast_days=1"
        )
        data = raw.json()
        ...
        image = graph.weather_graph(
            timestamps, temperature, humidity, dewpoint, rainfall
        )

        embed = nextcord.Embed(
            title=f"24-hour forecast",
            description="Data pulled from `open-meteo.com`",
            color=0x3346D1,
        )
        ...
        await interaction.followup.send(
            embed=embed, file=nextcord.File(image, filename="file.png")
        )
